...
def test(model, test_loader, criterion, device, epoch_no, hook):
    ...
    model.eval()
    hook.set_mode(smd.modes.EVAL) #Setting a debugger hook mode to EVAL
    running_loss = 0
    running_corrects = 0
    with torch.no_grad():
...
def train(model, train_loader, criterion, optimizer, device, epoch_no, hook):
    ...
    model.train()
    hook.set_mode(smd.modes.TRAIN) #Setting a debugger hook mode to EVAL
    running_loss = 0
    running_corrects = 0
    running_samples = 0
...
def net():
    model = models.resnet50(pretrained = True)  #Using the resnet50 pretrained model with 50 layers
    ...
    model.fc = nn.Sequential( nn.Linear( num_features, 256), #Adding two fully connected layers
                             ...
                             nn.Linear(256, 133),
                             nn.ReLU(inplace = True) #Output should have the same number of nodes as the number of dog breed classes (133)
                            )
    return model
